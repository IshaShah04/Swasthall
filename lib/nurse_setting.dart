...
class _NurseSettingState extends State<NurseSetting> {
  ...
  Future<void> _pickAndUploadImage() async {
    ...
    try {
      // Universal approach: Read bytes to support Web, iOS, and Android
      final bytes = await image.readAsBytes();
      final userId = supabase.auth.currentUser!.id;
      final path = 'avatars/avatar_$userId.jpg';

      // uploadBinary is the universal method for Supabase storage
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          _editableTile("My Speciality", specialityValue, 'nurse_speciality', Icons.badge_outlined),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: const Column(
        children: [
          ...
          Text("You are not linked to any doctor profile. Please contact the administrator.", 
    ...
    return Container(
      ...
      child: ListTile(
        ...
        subtitle: Text(name != null 
          ...
          : "ID: $id", style: const TextStyle(fontSize: 12)),
        trailing: name != null 
  ...
  Future<void> _saveSlot(String targetDocId, String type) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Doctor's $type slot saved!"), 
            backgroundColor: brandBlue
          ),
        );
      }
  ...
  Widget _sectionHeader(String title, {VoidCallback? onTrailingTap, IconData? trailingIcon, String? trailingLabel}) {
    return Row(
  ...
  Widget _actionBtn(String txt, Color col, VoidCallback onP) => ElevatedButton(
    ...
    onPressed: onP, child: Text(txt, style: const TextStyle(color: Colors.white))
  );
  ...
  Future<void> _deleteSlot(id) async => 
    ...
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              await supabase.from('nurse_staff_unified')
                  .update({column: controller.text})
                  .eq('nurse_email', supabase.auth.currentUser!.email!);
              await _syncData();
              if (mounted) {
                navigator.pop(); 
              }
